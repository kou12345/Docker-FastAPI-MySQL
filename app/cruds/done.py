...
async def get_done(db: AsyncSession, task_id: int) -> Optional[task_model.Done]:
    ...
    done: Optional[Tuple[task_model.Done]] = result.first()  # type: ignore
    return done[0] if done is not None else None  # 要素が1つであってもtupleが返されるので1つめの要素を取り出す


async def create_done(db: AsyncSession, task_id: int) -> task_model.Done:  # type: ignore
